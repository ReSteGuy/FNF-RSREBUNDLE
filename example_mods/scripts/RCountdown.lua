local thingy = 0 --or it was math?
local defcdsprite =  {"go", "set", "ready"}
local defcdpixelsprite = {"pixelUI/date-pixel", "pixelUI/set-pixel", "pixelUI/ready-pixel"}

function onCreatePost()
		for intro = 1, 3 do
			if getPropertyFromClass('PlayState', 'isPixelStage') == true then --no fucking idea why it works like this
...
function onResume()
	if curBeat > 1 then
		if not stoprightthereyoucriminalscum then
			pausecurcountdown = 4
			runTimer('pausecountdown', thingy)
...
function onTimerCompleted(tag, loops, loopsLeft)
	...
	if tag == 'pausecountdown' then
		if pausecurcountdown > 0 then	
			...
			doTweenAlpha('leintro'..pausecurcountdown,'introshit'..(pausecurcountdown+1), 0 , thingy)
		...
		if pausecurcountdown == 4 then		
			doTweenAlpha('fancypausebg','fancymf', 0, thingy * 6, 'quartInOut')	
		...
		if pausecurcountdown == 0 then	
			...
			runTimer('letthegamebeggingplease', thingy)
			doTweenAlpha('GO','introshit1', 0 , thingy)
		end
