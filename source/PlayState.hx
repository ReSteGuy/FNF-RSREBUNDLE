...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("pha.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function set_playbackRate(value:Float):Float
	{
		if(generatedMusic)
		{
			...
			if(mvocals != null) mvocals.pitch = value;
			FlxG.sound.music.pitch = value;
		}
		playbackRate = value;
	...
	{
		scoreTxt.text = 'Score: ' + songScore + ' | Hits: ' + songHits
		+ ' | Misses: ' + songMisses
		+ ' | Rating: ' + ratingName
	...
	public function setSongTime(time:Float)
	{
		...
		if (Conductor.songPosition <= mvocals.length)
		{
			mvocals.time = time;
			mvocals.pitch = playbackRate;
		}
		vocals.play();
		mvocals.play();
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.multiVoices)
			mvocals = new FlxSound().loadEmbedded(Paths.mvoices(PlayState.SONG.song));
		else
			mvocals = new FlxSound();
	...
	function resyncVocals():Void
	{
		...
		if (Conductor.songPosition <= mvocals.length)
		{
			mvocals.time = Conductor.songPosition;
			mvocals.pitch = playbackRate;
		}
		vocals.play();
		mvocals.play();
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Play Sound':
				FlxG.sound.play(Paths.sound(value1), Std.parseFloat(value2));
		}
